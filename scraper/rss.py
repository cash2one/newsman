            ...
            if entry.has_key('images') and entry['images']:
                ...
                if biggest:
                    try:
                        ...
                        image_data = StringIO(urllib2.urlopen(image['url']).read())

                        # hot news image
                        hot_web, hot_local = image_helper.scale_image(image=biggest, image_data = image_data, size_expected=HOT_IMAGE_SIZE, resize_by_width=True, crop_by_center=True, relative_path='%s_hotnews' % image_relative_path)
