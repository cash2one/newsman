            ...
            if entry.has_key('images') and entry['images']:
                ...
                if biggest:
                    try:
                        ...
                        # hot news image
                        hot_web, hot_local = image_helper.scale_image(image=biggest, size_expected=HOT_IMAGE_SIZE, resize_by_width=True, crop_by_center=False, relative_path='%s_hotnews' % image_relative_path)
                        entry['hot_news_image'] = hot_web if hot_web else None
                        entry['hot_news_image_local'] = hot_local if hot_local else None

                        # category image
                        category_web, category_local = image_helper.scale_image(image=biggest, size_expected=CATEGORY_IMAGE_SIZE, resize_by_width=True, crop_by_center=False, relative_path='%s_category' % image_relative_path)
                        ...
                        # thumbnail image
                        if float(biggest['width']) / float(biggest['height'])  >= THUMBNAIL_STYLE: # thumbnail_landscape_size
                            thumbnail_web, thumbnail_local = image_helper.scale_image(image=biggest, size_expected=THUMBNAIL_LANDSCAPE_SIZE, resize_by_width=True, crop_by_center=False, relative_path='%s_thumbnail' % image_relative_path)
                        else: # else thumbnail_portrait_size
                            thumbnail_web, thumbnail_local = image_helper.scale_image(image=biggest, size_expected=THUMBNAIL_PORTRAIT_SIZE, resize_by_width=True, crop_by_center=False, relative_path='%s_thumbnail' % image_relative_path)
