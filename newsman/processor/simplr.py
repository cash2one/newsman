...
# CONSTANTS
HIDDEN_IMAGE = {
    r'http://[\w]*.folha.uol.com.br': [('td', {'class': 'articleGraphicImage'}),
                                       ('div', {'class': 'image'}),
                                       ('table', {'class': 'fe'})],
    r'http://[\w]*.r7.com/': [('div', {'class': re.compile('edges')})],
    r'http://www.estadao.com.br': [
        ('div', {'class': re.compile('bb-md-noticia-foto', re.I)})],
    r'http://br.[\w]*.yahoo.com': [
        ('li', {'class': re.compile('photo first last', re.I)})],
    r'http://g1.globo.com': [
        ('div', {'class': re.compile('foto componente_materia', re.I)})],
    r'http://[\w]*.posttoday.com': [
        ('p', {'class': re.compile('centerThumbnail', re.I)})],
    r'http://[\w]*.matichon.co.th': [
        ('a', {'class': re.compile('lytebox', re.I)})],
    r'http://[\w]*.mthai.com': [
        ('img', {'class': re.compile('alignnone size-full wp-image')})],
    r'http://[\w]*.detik.com': [('div', {'class': re.compile('pic_artikel')})],
    r'http://[\w]*.antaranews.com': [('div', {'class': 'imgNews'})],
    r'http://www.metrotvnews.com': [('div', {'class': 'read-media left'})],
    r'http://www.tempo.co': [('div', {'class': 'konten-foto-travel'})],
    r'http://[\w]*.okezone.com': [('div', {'id': 'pt'}),
                                  ('div', {'class': 'detail-img fl'})],
    r'http://[\w]*.inilah.com': [('div', {'class': 'imgbox'})],
    r'http://sankei.jp.msn.com/': [('div', {'class': 'img250 imgright'})],
    r'http://www.cnn.co.jp/': [
        ('div', {'id': 'leaf_large_image', 'class': 'img-caption'})],
    r'http://news.goo.ne.jp/': [('p', {'class': 'imager'})],
    r'http://jp.reuters.com/': [
        ('td', {'id': "articlePhoto", 'class': "articlePhoto"})]}


class Simplr:
    regexps = {
        'unlikely_candidates': re.compile(
            "banner|breadcrumb|button|combx|comment|community|copyright"
            "|disqus|extra|foot|footer|header|menu|remark|rss|poll|share"
            "|shoutbox|sidebar|sponsor|sns|ad-break|agegate|pagination|pager"
            "|popup|posted|pr|tweet|twitter",
            re.I),
        'ok_maybe_its_a_candidate': re.compile(
            "and|article|body|column|content|main|post|primary|shadow", re.I),
        'positive': re.compile(
            "article|blog|body|content|entry|hentry|image|main|page"
            "|pagination|photo|post|story|text",
            re.I),
        'negative': re.compile(
            "banner|breadcrumb|combx|comment|com|contact|foot|footer|footnote"
            "|genre|logo|masthead|media|meta|outbrain|poll|pr|promo|ranking"
            "|related|scroll|share|shoutbox|sidebar|sponsor|shopping|tags"
            "|tool|widget|link",
            re.I),
        'extraneous': re.compile(
            "print|archive|comment|discuss|e["
            "\-]?mail|share|reply|all|login|sign|single",
            re.I),
        'div_to_p_elements': re.compile(
    ...
    def _get_article(self):
        try:
            ...
            for elem in self.html.findAll(True):
                ...
                if 'g1.globo.com' in self.url and elem.get(
                        'class') and elem.get(
                        'class') == 'materia-conteudo entry-content':
                    do_not_extract = True

                if self.regexps['unlikely_candidates'].search(
                        unlikely_match_string) and not self.regexps[
                    'ok_maybe_its_a_candidate'].search(
                        unlikely_match_string) and elem.name != 'body':
                    # print elem.name
                    # print self.regexps['unlikely_candidates'].findall(
                    # unlikely_match_string)
                    # print 'id', elem.get('id') if elem.get('id') else None
                    # print 'class', elem.get('class') if elem.get('class')
                    # else None
                    # print '++++++++++++++++++++++++++++++++++++++++++++'
                    # print
                ...
                if self.url.startswith(
                        'http://community.travel.yahoo.co.jp/') and re.compile(
                        'pt02|pt10', re.I).search(unlikely_match_string):
                    elem.extract()
                    continue

                if self.url.startswith('http://news.nifty.com/') and re.compile(
                        'pr|banner', re.I).search(unlikely_match_string):
                    elem.extract()
                    continue
                ...
                if 'sanook.com' in self.url and re.compile(
                        'lts|fanpage|nav|wb-ch-h|wb-ly|article-info|related',
                        re.I).search(unlikely_match_string):
                    elem.extract()
                    continue

                if 'mthai.com' in self.url and re.compile(
                        'entry-other|source|profile-relate|profile-ig|news'
                        '-relate|news-tag',
                        re.I).search(unlikely_match_string):
                    elem.extract()
                    continue

                if 'okezone.com' in self.url and re.compile(
                        'subkanal|date-time fl|author fl', re.I).search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if 'tempo.co' in self.url and re.compile(
                        'title|submitted|inside-160|zoom-font|blok-share|blok'
                        '-sitemap|block-inner',
                        re.I).search(unlikely_match_string):
                    elem.extract()
                    continue

                if 'antaranews.com' in self.url and re.compile(
                        'mt10|datetime|byline|title_related|box_v',
                        re.I).search(unlikely_match_string):
                    elem.extract()
                    continue

                if 'detik.com' in self.url and re.compile(
                        'komentar|l_red2_detik|l_green_detik|author|date'
                        '|infoanda|articleshare2|iklan_tengah|adpoint_bottom'
                        '|recommend|box_1|info_redaksi|link_block',
                        re.I).search(unlikely_match_string):
                    elem.extract()
                    continue

                if 'posttoday.com' in self.url and re.compile('text-size',
                                                              re.I).search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if 'g1.globo.com' in self.url and re.compile(
                        'materia-cabecalho|materia-assinatura|materia-titulo',
                        re.I).search(unlikely_match_string):
                    elem.extract()
                    continue

                if re.compile('br.[\w]*.yahoo.com').search(
                        self.url) and re.compile('action enlarge', re.I).search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if re.compile('estadao.com.br').search(self.url) and re.compile(
                        'bb-md-noticia-autor', re.I).search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if 'ig.com.br' in self.url and re.compile(
                        'selo-agencia|tool-bar-buttons-social|galleria-image'
                        '-nav|galleria').search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if re.compile('http://[\w]*.terra.com.br').search(
                        self.url) and re.compile(
                        'embbed-gallery|score-widget|widget-container|col'
                        '-aside|ttl-main|published|related-news',
                        re.I).search(unlikely_match_string):
                    elem.extract()
                    continue

                if 'tecnoblog.net' in self.url and re.compile(
                        'breadcats|main-post-info|posttags|related-title'
                        '|yarpp-related').search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if 'canaltech.com.br' in self.url and re.compile(
                        'top|article-footer|block more|comments|tags').search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if 'rockntech.com.br' in self.url and re.compile(
                        'bottom-posts|comment|right').search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if 'codigofonte.net' in self.url and re.compile(
                        'containerMae|hierarquia|cabecalhoimg|ordem'
                        '|dataArtigo|tituloPagina|autorArtigo|outrosArtigos'
                        '|comentarios').search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if 'minhavida.com.br' in self.url and re.compile(
                        'sobre|colunaLateral|rodapeMV').search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if re.compile('http://[\w]*.r7.com').search(
                        self.url) and re.compile('column r7cl2 r7omega',
                                                 re.I).search(
                        unlikely_match_string):
                    elem.extract()
                    continue

                if re.compile('http://[\w]*.folha.uol.com.br').search(
                        self.url) and re.compile('gallery', re.I).search(
                        unlikely_match_string):
                    elem.extract()
                    continue
                ...
                if elem.name == 'div':
                    ...
                    if self.url.startswith('http://jp.reuters.com/'):
                        ...
                        if elem.parent.name == 'div':
                            if elem.name == 'img' and elem.parent.get(
                                    'id') and (elem.parent.get(
                                    'id') == 'article' or elem.parent.get(
                                    'id') == 'container2'):
                                elem.name = 'p'
                            else:
                                pass
                    elif 'estadao.com.br' in self.url:
                        if elem.get('style') and elem.get(
                                'style') == 'font-size: 14px;':
                            elem.extract()
                        elif elem.name == 'embed' and elem.get(
                                'type') and elem.get(
            ...
            for node in self.html.findAll('p'):
                ...
                if parent_hash not in self.candidates:
                    ...
                    # print 'pn-class', parent_hash, parent_node.get(
                    # 'class'), self._get_class_weight(parent_node)
                ...
                if grand_parent_node and grand_parent_hash not in self \
                    ...
                    self.candidates[grand_parent_hash] = self._initialize_node(
                        grand_parent_node)
                    # print 'gpn-class', grand_parent_hash, grand_parent_node
                    # .get('class'), self._get_class_weight(grand_parent_node)
                ...
                # print 'OLD %s %s  Parent' % (self.candidates[parent_hash][
                # 'score'], parent_hash)
                # print 'OLD %s %s  Grand' %
                # (self.candidates[grand_parent_hash]['score'],
                # grand_parent_hash)
                ...
                if grand_parent_node:
                    ...
                    # print 'NEW %s %s  Parent' % (self.candidates[
                    # parent_hash]['score'], parent_hash)
                    # print 'NEW %s %s  Grand' % (self.candidates[
                    # grand_parent_hash]['score'], grand_parent_hash)
                    # print node.parent
                    # print
                    # '=================================================='
                    # print '@@@@@'
        ...
        try:
            ...
            for target in target_list:
                ...
                if is_embed and self.regexps['videos'].search(
                        target.renderContents(encoding='utf-8')):
                    continue
                target.extract()
        except Exception as k:
            logger.error(str(k))
    ...
    def _clean_extra_parts(self, e):
        try:
            ...
            if 'antaranews.com' in self.url:
                unwanted_parts = u"COPYRIGHT . [\d]{3}|Ikuti berita terkini " \
                                 u"di handphone anda di"
            if 'detik.com' in self.url:
                unwanted_parts = u"Your Browser didn't support iframe"
            if 'sanook.com' in self.url:
                unwanted_parts = \
                    u'อ่านเรื่องที่เกี่ยวข้องทั้งหมด|ร่วมเป็นแฟนเพจเรา บน'
            if 'mthai.com' in self.url:
                unwanted_parts = u'MThai News'
            if 'matichon.co.th' in self.url:
                unwanted_parts = u'facebook' \
                                 u'.com/MatichonOnline|ร่วมเป็นแฟนเพจเฟซบุ๊กกับมติชนออนไลน์'
            ...
            if 'ig.com.br' in self.url:
                unwanted_parts = u'LEIA TAMBÉM|SAIBA MAIS|Veja também|Leia ' \
                                 u'mais|E também|Assista|Infográfico|veja foto'
            ...
            if unwanted_parts:
                ...
                for extra_part in extra_parts:
                    if extra_part.parent.find('a'):
                        ...
                        extra_part.extract()
                        #[extra_part.parent.extract() for extra_part in
                        # extra_parts]
            ...
            if len(unwanted_spaces) == 1:
                if len(unwanted_spaces[0].contents) == 0:
                    ...
                    if not isinstance(unwanted_spaces[0].contents[0],
                                      NavigableString) and len(
                            unwanted_spaces[0].contents[0].text) < 15:
                        unwanted_spaces[0].extract()
                    elif isinstance(unwanted_spaces[0].contents[0],
                                    NavigableString) and len(
                            unwanted_spaces[0].contents[0]) < 15:
                        unwanted_spaces[0].extract()
            ...
            for unwanted_space in unwanted_spaces:
                ...
                if len(unwanted_space.contents) == 1:
                    ...
                    if not isinstance(unwanted_space.contents[0],
                                      NavigableString):
                        # print '      --', unwanted_space.contents[0]
                        if unwanted_space.contents[0].name == ['br', 'strong',
                                                               'hr', 'footer',
                                                               'div', 'p']:
                            # print '      DELETED!'
