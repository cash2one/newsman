...
for item in items:
    # if 'status' in item and item['status'] and (item['status'] == 200 or
    # item['status'] == 302 or item['status'] == 304):
    # pass
    ...
    if not item['updated_times']:
        print item['language'], str(item['feed_title']), "[%s]" % item[
            'latest_update'] if 'latest_update' in item and item[
            'latest_update'] else '-', '[%s]' % item[
            'reason'] if 'reason' in item and item['reason'] else '-', \
            '[%s]' % \
                                                                       item[
                                                                           'status'] if 'status' in item and \
                                                                                        item[
                                                                                            'status'] else '-'
        col.remove({'_id': item['_id']})
